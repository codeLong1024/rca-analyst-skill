...
- **证据纪律** - 四象限证据采集（语义/结构/运行时/变更），静态推断必须被运行时证据证实，无证据不下结论
- **结构化判定** - 否定优先排除误报后，映射到封闭判定枚举（fix_code / fix_config / rate_limit / whitelist），置信度三档评分
- **结构化输出** - 标准化 Markdown 报告：5 Whys 分析、证据四象限、判定结果与置信度、方案对比、实施路线图
...
rca-analyst-skill/
├── SKILL.md       # 技能定义（入口文件）
├── WORKFLOW.md    # 分析工作流指南（铁律 + 四象限证据 + 5 Whys + 判定枚举 + 置信度）
├── LICENSE
└── README.md
```
